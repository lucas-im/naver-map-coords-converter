...
const convertCoors = (oriCoords) => {
    ...
    return Object(proj4__WEBPACK_IMPORTED_MODULE_1__["default"])(naverMap3857Projion, wgs84Projection, oriCoords)}
    catch(e) {
        alert(`주소 변환 실패!{e}`)
    }
}
const buttonClicked = (coords) => {
    ...
    const observer = new MutationObserver(mutation => {
        mutation.forEach((e) => {
            if(e.target.className  === 'entry_wrap loaded'){
                ...
                newText.innerHTML = newCoords
                console.log(newText)
                placeHolder.append(newText)
                newText.onclick = buttonClicked(newCoords)
                newText.style.backgroundColor = '#FFCCCB'
                // newText.addEventListener('click', buttonClicked(newCoords))
                return
            }
        })
    })
